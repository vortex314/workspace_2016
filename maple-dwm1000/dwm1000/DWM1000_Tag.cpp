...
//_________________________________________________ delay in msec
//
void delay_tag(uint32_t msec) {
...
void DWM1000_Tag::setup() {
	...
	_pinIrq->setInterrupt(Gpio::GPIO_EDGE_FALLING, []() {
		if ( _gDwm1000_tag ) _gDwm1000_tag->onIrq();
	});
	timeout(1000);
}
...
void DWM1000_Tag::onEvent(Cbor& msg) {
	...
	POLL_SEND: {
		while (true) {
			...
			timeout(10);
			PT_YIELD_UNTIL(event == H("timeout") || isInterruptDetected());	// WAIT RESP MSG

			if (isInterruptDetected())
				LOGF(" INTERRUPT DETECTED ");

			status_reg = dwt_read32bitreg(SYS_STATUS_ID);
			LOGF(" SYS_STATUS : 0x%X", status_reg);
	...
	RESP_RECEIVED: {
		...
		if (memcmp(rx_buffer, rx_resp_msg, ALL_MSG_COMMON_LEN) == 0) {// CHECK RESP MSG
			...
			timeout(10);
			PT_YIELD_UNTIL(
					(dwt_read32bitreg(SYS_STATUS_ID) & SYS_STATUS_TXFRS) || timeout());
			/* Clear TXFRS event. */
