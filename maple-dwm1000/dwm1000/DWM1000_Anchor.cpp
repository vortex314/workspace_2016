...
DWM1000_Anchor* _gDwm1000_anchor;

DWM1000_Anchor::DWM1000_Anchor() {
	_gDwm1000_anchor = this;
	_count = 0;
}
...
void DWM1000_Anchor::init() {
	...
	LOGF(" device id : 0x%X , part id : 0x%X, lot_id : 0x%X", device_id,
			part_id, lot_id);
...
//			dwt_setinterrupt(DWT_INT_RFCG, 1);	// enable RXD interrupt
			...
			while (true) { /* Poll for reception of a frame or error/timeout. See NOTE 7 below. */
				...
				clearInterrupt();
				PT_YIELD_UNTIL(
						(event == H("timeout")) || isInterruptDetected());
				status_reg = _status_reg;
				LOGF(" status reg.: %X ,interrupts : %X", status_reg,
						interruptCount);
				status_reg = dwt_read32bitreg(SYS_STATUS_ID);
				LOGF(" IRQ pin : %X status_reg DWM1000 %X ", getInterrupt(),
						status_reg); // PULL LOW
		...
		if (status_reg & SYS_STATUS_RXFCG) {
			...
			if (frame_len <= RX_BUF_LEN) { // was RX_BUFFER_LEN
...
//				PT_YIELD_UNTIL(timeout() || isInterruptDetected());
				status_reg = dwt_read32bitreg(SYS_STATUS_ID);
//				status_reg = _status_reg;
				LOGF(" status reg2: 0x%X", status_reg);
//					if (status_reg & (SYS_STATUS_RXFCG | SYS_STATUS_ALL_RX_ERR))
//						break;
//				}
...
//						sprintf(dist_str,"%3.2f", distance);
						...
						LOGF(" distance : %f m. ", (float )distance);
					}
				} else {
